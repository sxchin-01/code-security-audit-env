...
from app.env import CodeSecurityAuditEnv
...
def _action_from_llm(*, api_key: str, model: str, observation: Observation) -> Action:
...
def run_baseline(model: str = "deepseek-ai/DeepSeek-R1:fastest") -> None:
    """Run one deterministic pass over the full task list and print scores."""

    api_key = os.getenv("HF_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("HF_API_KEY is required to run inference.py")
    if "your_" in api_key.lower() or "replace" in api_key.lower() or "hf-your" in api_key.lower():
        raise RuntimeError(
            "HF_API_KEY looks like a placeholder. Set a real token and try again."
        )
    ...
    print(f"Running baseline on {task_count} tasks with model={model}")
    ...
    for idx in range(task_count):
        ...
        while not done:
            step_index += 1
            try:
                action = _action_from_llm(api_key=api_key, model=model, observation=observation)
        ...
        # Keep final task score normalized to [0, 1].
        final_task_score = max(0.0, min(1.0, episode_reward))
        task_scores.append(final_task_score)

        print(
            f"[task {idx + 1}/{task_count}] task_id={observation.task_id} "
            f"difficulty={observation.difficulty.value} score={final_task_score:.4f}"
        )

    average_score = sum(task_scores) / len(task_scores) if task_scores else 0.0

    print("\n=== Summary ===")
    for i, score in enumerate(task_scores, start=1):
        print(f"Task {i}: {score:.4f}")
